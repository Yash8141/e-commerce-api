export const validateProductQuery = query => {
  ...
  const validatedSearchBy = validSearchFields.includes(searchBy) ? searchBy : "title";

  // Validate sort parameter
  const validateSortDirections = ["asc", "desc"];
  const validSortFields = ["title", "price", "category", "createdAt", "updatedAt"];
  const validatedSortDir = validateSortDirections.includes(sortDir.toLowerCase()) ? sortDir.toLowerCase() : "desc";
